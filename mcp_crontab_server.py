...
def fetch__crontab_contents():
    ...
    print(f"Tool call: fetch_crontab_entries at {datetime.datetime.now()}")
    try:
        command = "crontab -l"
        ...
        if result.returncode == 0 and result.stdout:
            ...
            return "No cron jobs found or error occurred."
    except Exception as e:
        print(f"Error in fetch_crontab_entries: {str(e)}")
...
def show_scheduled_task_summary():
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout:
            ...
            return "No recent cron logs found or error occurred."
    except Exception as e:
...
def search_crontab_entries(search_term):
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout:
            ...
            return f"No crontab entries found containing '{search_term}' or error occurred."
    except Exception as e:
